...
export function extractPlaylistId(url: string): string | null {
  ...
  // Pattern 1: playlist?list=PLAYLIST_ID
  let match = trimmed.match(/[?&]list=([a-zA-Z0-9_-]+)/);
  if (match) {
    return match[1];
  }

  // Pattern 2: /playlist/PLAYLIST_ID (less common)
  match = trimmed.match(/\/playlist\/([a-zA-Z0-9_-]+)/);
  if (match) {
    return match[1];
  }
...
export function validateAndNormalizeYouTubeUrl(url: string): {
  ...
  if (!videoId) {
    ...
    const isYouTubeDomain = /youtube\.com|youtu\.be/.test(trimmed);
    if (isYouTubeDomain) {
      return { valid: false, error: 'Không thể extract video ID từ URL' };
    }
