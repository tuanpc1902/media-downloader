...
interface FacebookInputSectionProps {
...
/**
 * Facebook URL Input Section
 * 
 * Features:
 * - Multi-line URL input
 * - Real-time validation (Facebook URLs only)
 * - Drag & drop support
 * - Paste from clipboard
 * - Visual validation indicators
 */
export function FacebookInputSection({ onAnalyze, onClear, loading = false }: FacebookInputSectionProps) {
  const [text, setText] = useState('');
  const [isDragging, setIsDragging] = useState(false);
  const [validationResults, setValidationResults] = useState<Map<number, FacebookURLValidationResult>>(new Map());
  ...
  const validateFacebookURL = useCallback((url: string): FacebookURLValidationResult => {
    ...
    if (!trimmed.includes('facebook.com') && !trimmed.includes('fb.com') && !trimmed.includes('m.facebook.com')) {
      return { 
        isValid: false, 
        url: trimmed, 
        error: 'Chỉ hỗ trợ URL Facebook. Ví dụ: https://www.facebook.com/username/videos/1234567890' 
      };
    }
  ...
  useEffect(() => {
    ...
    const results = new Map<number, FacebookURLValidationResult>();
    
    urls.forEach((url, index) => {
      results.set(index, validateFacebookURL(url));
    });
    
    setValidationResults(results);
  }, [text, parseURLs, validateFacebookURL]);
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        <textarea
          ...
          placeholder="Nhập URL Facebook video/story... (hỗ trợ nhiều URL, mỗi dòng một URL)&#10;&#10;Ví dụ:&#10;https://www.facebook.com/username/videos/1234567890"
          className={cn(
      ...
      {hasValidURLs && (
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-600 dark:text-gray-400">
            {validURLs.length} video/story{validURLs.length > 1 ? 's' : ''} ready to analyze
          </span>
        </div>
      )}
      ...
      <p className="text-xs text-gray-500 dark:text-gray-400">
        💡 Tip: Bạn có thể nhập nhiều URL Facebook, mỗi URL một dòng hoặc phân cách bằng dấu phẩy. 
        Hỗ trợ drag & drop URL hoặc paste từ clipboard. Hỗ trợ video và story.
      </p>
    </div>
  );
}
